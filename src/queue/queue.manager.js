...
class QueueManager {
  ...
  async init() {
    ...
    try {
      staticQueue = new BullMQ.Queue(STATIC_QUEUE, {
        connection,
        defaultJobOptions: JOB_DEFAULTS,
      });
      dynamicQueue = new BullMQ.Queue(DYNAMIC_QUEUE, {
        connection,
        defaultJobOptions: JOB_DEFAULTS,
      });
  ...
  async getJob(jobId) {
    ...
    const [staticJob, dynamicJob] = await Promise.all([
      BullMQ.Job.fromId(this.staticQueue, jobId).catch(() => null),
      BullMQ.Job.fromId(this.dynamicQueue, jobId).catch(() => null),
    ]);
    return staticJob ?? dynamicJob ?? null;
  }
  ...
  async getStats() {
    ...
    try {
      const [s, d] = await Promise.all([
        ...
        this.dynamicQueue.getJobCounts(
          "waiting",
          "active",
          "completed",
          "failed",
