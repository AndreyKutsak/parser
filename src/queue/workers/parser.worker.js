...
const SHUTDOWN_TIMEOUT_MS =
  parseInt(process.env.WORKER_SHUTDOWN_TIMEOUT_MS) || 30_000;
...
async function main() {
  ...
  const jobHandler = async (job) => {
    ...
    if (!task) {
      logger.warn("Job references deleted task, removing from queue", {
        jobId: job.id,
        taskId,
      });
      await job.remove();
  ...
  const staticWorker = new Worker(
    STATIC_QUEUE,
    jobHandler,
    workerOpts(STATIC_CONCURRENCY),
  );
  const dynamicWorker = new Worker(
    DYNAMIC_QUEUE,
    jobHandler,
    workerOpts(DYNAMIC_CONCURRENCY),
  );
  ...
  for (const worker of [staticWorker, dynamicWorker]) {
    ...
    worker.on("stalled", (jobId) => {
      logger.warn("Job stalled — will be retried or moved to failed", {
        jobId,
      });
    });
    worker.on("error", (err) => {
